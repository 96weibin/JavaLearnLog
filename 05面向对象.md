...
public class Point {
	int x;
	int y;
	public Point (int x) {    //新的构造器？？
		this(x,x);// this()   调用了 构造器 传参x x
		//this.x = x;
		//this.y = y;
	}
...
static 的会在类加载的时候执行、 static 的属性、方法 不需要new 实例  可以直接  通过  类名.属性来访问
...
1. final 修饰的类不能被继承
	...
	* 实际项目中，原则不允许使用final类（Spring等框架使用了动态继承代理技术  使用final得嘞会影响实现）
2. final修饰的方法 不会被覆盖
3. final 修饰的变量 初始化后 不允许修改
4. 同时使用 final static修饰的变量是常量
	
	```java
		final static Java	//常量名 要有大写字母(这是社么奇葩设定)
	```
### abstract class
...
3. 接口不能实例化
4. 接口变量可以引用具体实现类的实例
5. 接口只能被实现(继承)、一个具体类实现接口必须使用全国不的抽象方法
6. 接口之间可以继承
